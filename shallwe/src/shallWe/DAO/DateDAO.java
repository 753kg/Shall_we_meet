...
public class DateDAO {
   ...
   public List<DateVO> selectAllDates(String planid){
      ...
      try {
         ...
         while(rs.next()) {
            dlist.add(new DateVO(planid, rs.getString(1), rs.getString(2)));
         }
      } catch (SQLException e) {
         e.printStackTrace();
      }finally {
   ...
   public DateCountVO selectCountNDate(String planid){
      DateCountVO dvo = new DateCountVO();
      String sql = 
            " select max(cnt), max(select_date) "
         +   " from ( "
         +        "   select select_date, count(member_id) as cnt "
         +        "   from dates "
         +        "   where plan_id = ?"
         +         " group by select_date )";
